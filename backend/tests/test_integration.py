...
class TestFullWorkflow:
    ...
    async def test_full_detection_workflow(self, client: AsyncClient):
        """测试完整检测流程"""
        # 1. 创建摄像头
        ...
        # 4. 验证事件列表包含此事件
        events_list = await client.get("/api/events", params={"camera_id": "INT_CAM001"})
        assert events_list.status_code == 200
        assert len(events_list.json()) >= 1

        # 5. 验证告警列表
        alerts_list = await client.get("/api/alerts")
        assert alerts_list.status_code == 200
        assert len(alerts_list.json()) >= 1

        # 6. 确认告警
        ack_resp = await client.put(f"/api/alerts/{alert['id']}/acknowledge", json={
        ...
        # 7. 查看看板汇总
        dash = await client.get("/api/stats/dashboard")
        assert dash.status_code == 200

    async def test_multi_camera_events(self, client: AsyncClient):
        """多摄像头并发检测事件"""
        # 创建两个摄像头
        ...
        # 按类型过滤
        safety = await client.get("/api/events", params={"event_type": "safety"})
        assert safety.status_code == 200
        assert all(e["event_type"] == "safety" for e in safety.json())

    async def test_camera_lifecycle(self, client: AsyncClient):
        """摄像头完整生命周期"""
        # 创建
        resp = await client.post("/api/cameras", json={
        ...
        # 更新
        up = await client.put(f"/api/cameras/{cam_id}", json={
            "name": "已更新摄像头",
            "status": "offline",
        })
        ...
        # 删除
        dl = await client.delete(f"/api/cameras/{cam_id}")
        assert dl.status_code == 204

        # 验证已删除
        get = await client.get(f"/api/cameras/{cam_id}")
        assert get.status_code == 404

    async def test_alert_stats_accuracy(self, client: AsyncClient):
        """告警统计准确性"""
        # 创建不同级别告警
        ...
        await client.put(f"/api/alerts/{first_id}/acknowledge", json={
            "acknowledged_by": "admin",
        })

        # 验证统计
        stats = await client.get("/api/alerts/stats")
